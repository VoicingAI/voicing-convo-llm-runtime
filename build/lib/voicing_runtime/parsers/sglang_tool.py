...
"""Voicing tool-call detector for SGLang.

Adapted for Voicing AI from the SGLang XML function-call detector and kept in
step with the `Voicing-Convo-V2-35B-MOE` chat template, which emits:
...
Registers itself under the tool-call parser name ``voicing``.

Based on the detector shipped in SGLang 0.5.16 -- it depends only on
``BaseFormatDetector`` and ``core_types``, which are stable across 0.5.x and
``main``; newer upstream revisions pull helpers from ``function_call.utils``
that older releases do not have.
"""
...
class VoicingToolDetector(BaseFormatDetector):
    def __init__(self):
        super().__init__()
    ...
    def get_structural_tag_name(self) -> str:
        ...
        # grammar-constrained tool calling. This is an inference-backend
        # grammar id, not a brand name - it must match a template name xgrammar
        # ships, so it is deliberately left unchanged.
        return "qwen_3_coder"
...
FunctionCallParser.ToolCallParserEnum["voicing"] = VoicingToolDetector

__all__ = ["VoicingToolDetector"]
