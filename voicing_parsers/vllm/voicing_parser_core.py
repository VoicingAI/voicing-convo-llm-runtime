...
"""Voicing parser for tool calls and reasoning (vLLM).

Adapted for Voicing AI from the vLLM parser-engine reference implementation and
kept in step with the `Voicing-Convo-V2-35B-MOE` chat template.

Based on the parser shipped in vLLM 0.28.0. It uses only ParserEngineConfig
fields that exist in that release; newer upstream revisions add optional
fields (e.g. turn-boundary tokens) that older releases reject, so this file
deliberately tracks the released API and runs unchanged on `main`.

Voicing XML tool call format::
...
``_voicing_arg_converter`` parses these into a JSON object.
"""
...
def _trim_wrapping_newlines(value: str) -> str:
    """Strip one leading and one trailing newline (the chat template markup)."""
    if value.startswith("\n"):
        value = value[1:]
    if value.endswith("\n"):
        value = value[:-1]
...
def _voicing_arg_converter(raw_args: str, partial: bool) -> str:
...
def voicing_config(
    ...
    *,
    ...
    return ParserEngineConfig(
        ...
        arg_converter=_voicing_arg_converter,
...
class VoicingParser(ParserEngine):
    """Voicing parser: ``<think>``/``</think>`` reasoning +
